...
# 4) Permanent SEO smoke: do not confuse Cloudflare bot-specific Disallow:/
# with a global search-engine block. Add regressions for the discovered issues.
p = Path(".github/workflows/seo-indexability-smoke.yml")
s = p.read_text()
old = '''            if grep -Eq '^Disallow:[[:space:]]*/[[:space:]]*$' "$robots"; then
...
'''
if old not in s:
    raise SystemExit("robots false-positive block not found")
s = s.replace(old, '''            # Cloudflare may add Disallow: / for named AI crawlers. Generic
            # search crawling is verified by Allow: / plus real Googlebot page checks.
''', 1)

marker = '''            check_indexable_page '/yayinevleri-icin' 'https://ilkoku.com/yayinevleri-icin' || { cleanup; return 1; }
...
'''
addition = '''            check_indexable_page '/yayinevleri-icin' 'https://ilkoku.com/yayinevleri-icin' || { cleanup; return 1; }

            local www_headers www_code www_location filtered filtered_code
            www_headers="$(mktemp)"
            ...
            rm -f "$www_headers"
            if [[ "$www_code" != "301" && "$www_code" != "308" ]] || [[ "$www_location" != "https://ilkoku.com/" ]]; then
              echo "FAIL: www canonical redirect code=$www_code location=${www_location:-YOK}"
              cleanup
              return 1
            fi

            for filtered in 'https://ilkoku.com/eserler?arama=test' 'https://ilkoku.com/yazarlar?arama=test' 'https://ilkoku.com/turler?arama=test'; do
              filtered_code="$(curl --silent --show-error --location --max-time 25 --user-agent "$USER_AGENT" --output /tmp/seo-filtered --write-out '%{http_code}' "$filtered" || true)"
              if [[ "$filtered_code" != "200" ]]; then
                echo "FAIL: filtered discovery URL returned HTTP $filtered_code: $filtered"
                cleanup
                return 1
              fi
              if ! grep -Eqi '<meta[^>]+name="robots"[^>]+content="[^"]*noindex' /tmp/seo-filtered; then
                echo "FAIL: filtered discovery URL missing noindex: $filtered"
                cleanup
                return 1
              fi
            done
            rm -f /tmp/seo-filtered

            curl --fail --silent --show-error --location --max-time 25 --user-agent "$USER_AGENT" https://ilkoku.com/eserler --output /tmp/seo-eserler || { echo "FAIL: eserler social metadata fetch"; cleanup; return 1; }
            grep -Eqi '<meta[^>]+property="og:image"' /tmp/seo-eserler || { echo "FAIL: eserler og:image missing"; rm -f /tmp/seo-eserler; cleanup; return 1; }
            rm -f /tmp/seo-eserler

            curl --fail --silent --show-error --location --max-time 25 --user-agent "$USER_AGENT" https://ilkoku.com/ --output "$home" || { echo "FAIL: homepage schema fetch"; cleanup; return 1; }
'''
if marker not in s:
    raise SystemExit("SEO smoke insertion marker not found")

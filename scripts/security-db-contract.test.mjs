...
test("fresh migrations preserve critical unique concurrency constraints", async () => {
  const publisherEditorIndexes = await uniqueIndexes(
    "PublisherEditorRequest",
  );
  const emailDedupeIndexes = await uniqueIndexes(
    "EmailDeliveryDedupe",
  );
...
test("FOR UPDATE keeps eligibility state stable for the active transaction", async () => {
  ...
  try {
    ...
    assert.equal(
      changerSettled,
      false,
      "a competing state change must wait while FOR UPDATE is held",
    );

    await locker.commit();
    const affected = await changePromise;
...
test("unique active key rejects a concurrent duplicate insert", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      [...results].sort(),
      ["duplicate", "inserted"],
      "the unique active key must permit only one concurrent insert",
    );
  } finally {
    await dropTable(table);
  }

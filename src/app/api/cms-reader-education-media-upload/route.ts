...
export async function POST(request: Request) {
  ...
  const folder = readerEducationMediaFolder(category);
  const githubSourceFolder = readerEducationGithubMediaFolder(category);
  ...
  const assetPayload = JSON.stringify({
    ...
    altText,
    kind: "image",
    collection: "reader-education",
    folder,
    githubSourceFolder,
    readerEducationSlug: category.slug,
    slot: slotInfo.key,
    ...
    usage: `Okur Eğitimi / ${category.title} / ${slotInfo.number} ${slotInfo.label}`,
    ...
    await prisma.$transaction([
      ...
      prisma.$executeRaw`
        INSERT INTO SiteContent (
          ...
          ${randomUUID()}, ${READER_EDUCATION_GUIDE_NAMESPACE}, ${category.slug}, ${guidePayload}, 'json', 'published', CURRENT_TIMESTAMP(3),
          ${access.user.id}, CURRENT_TIMESTAMP(3), CURRENT_TIMESTAMP(3)
        )
  ...
  response.headers.set("x-reader-education-public-path", readerEducationPublicPath(category));
  response.headers.set("x-reader-education-media-folder", folder);
  return response;
}

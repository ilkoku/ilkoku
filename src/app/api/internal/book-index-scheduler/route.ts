...
export async function POST(request: NextRequest) {
  ...
  try {
    const forceIlla = request.nextUrl.searchParams.get("forceIlla") === "1";
    let forcedIllaRun: Awaited<ReturnType<typeof collectBookIndexListByCode>> | null = null;

    if (forceIlla) {
      forcedIllaRun = await collectBookIndexListByCode("illakitap-tr-weekly");
    }

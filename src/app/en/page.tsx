...
export default async function EnglishHomePage() {
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#f8f7fb", color: "#211f2b" }}>
      ...
      {roles?.title ? <section id="roles" style={{ maxWidth: 1080, margin: "0 auto", padding: "3rem 1.25rem" }}><span style={{ color: "#6847e8", fontWeight: 800 }}>{roles.eyebrow}</span><h2 style={{ fontSize: "clamp(2rem,5vw,3.2rem)" }}>{roles.title}</h2><p style={{ color: "#666171", lineHeight: 1.7 }}>{roles.description}</p><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: ".8rem", marginTop: "1.5rem" }}>{["Writer","Reader","Editor","Publisher"].map((role) => <div key={role} style={{ padding: "1.2rem", borderRadius: "1rem", background: "white", border: "1px solid #e9e5f0", fontWeight: 800 }}>{role}</div>)}</div></section> : null}

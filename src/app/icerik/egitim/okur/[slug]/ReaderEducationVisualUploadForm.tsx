...
export default function ReaderEducationVisualUploadForm({
  categorySlug,
  slotKey,
  slotLabel,
  recommendedWidth,
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!check.ratioValid) {
      setError(`Görsel oranı ${slotLabel} alanına uygun değil. Hedef oranı koruyan bir görsel seçin.`);
      return;
    }
    ...
    xhr.onload = () => {
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        window.location.assign(`/icerik/egitim/okur/${categorySlug}?yuklendi=${slotKey}`);
        return;
      }
      setUploading(false);
      setProgress(null);
    ...
    xhr.open("POST", "/api/cms-reader-education-media-upload");
    xhr.send(data);
  }

...
function readinessItems(data: CmsReadinessSnapshot, canPublish: boolean): ReadinessItem[] {
  ...
  const guideAge = pendingAge(data.guidesPendingAgeHours);
  ...
  return [
    ...
    { label: "Yasal Sayfalar", value: `${data.legal}/${cmsReadinessTargets.legal}`, level: data.legal >= cmsReadinessTargets.legal ? "pass" : "blocker", detail: "Kullanım, Gizlilik, KVKK, Çerez ve Telif metinlerinin CMS sahipliği.", href: "/icerik/yasal?dil=tr", action: "Belgeleri aç" },
    {
      ...
      href: corporateHref,
      action: data.corporate >= cmsReadinessTargets.corporate ? data.corporatePendingDraft > 0 ? canPublish ? "Değişikliği incele/yayınla" : "Değişikliği incele" : "Kaydı aç" : data.corporateCreated >= cmsStarterTargets.corporate ? canPublish ? "İncele ve yayınla" : "Taslağı incele" : data.corporateArchived > 0 ? "Arşiv kaydını aç" : "Başlangıç setini aç",
    ...
    },
    {
      ...
      action: data.faq >= cmsReadinessTargets.faq ? data.faqPendingDrafts > 0 ? canPublish ? "Değişikliği incele/yayınla" : "Değişikliği incele" : "Temel SSS'leri aç" : data.faqCreated > 0 || data.faqArchived > 0 ? canPublish ? "İncele ve yayınla" : "Taslakları incele" : "SSS modülünü aç",
    ...
    { label: "Yayın Kuyruğu", value: String(data.queue), level: data.queue === 0 ? "pass" : "info", detail: data.queue === 0 ? "Bekleyen taslak yok." : "Operasyon kuyruğunda taslaklar var. Bu kayıtlar mevcut canlı içeriğin yayın kabulünü bozmaz; ayrı olarak incelenip planlanır.", href: "/icerik/yayin-kuyrugu", action: data.queue === 0 ? "Kuyruğu aç" : "Taslakları yönet" },
  ];
}
        ...
        <>
          ...
          {starter.pendingTotal > 0 ? (
            <div className="content-panel" style={{ marginTop: "1rem" }} role="status">
              <div className="content-section-heading"><div><span>Editoryal Takip</span><h2>Yayındaki içerikte {starter.pendingTotal} bekleyen temel değişiklik var</h2></div><p>{oldestPendingAge ? `En eski değişiklik ${oldestPendingAge}dır bekliyor.` : "Canlı kabul etkilenmiyor."}</p></div>

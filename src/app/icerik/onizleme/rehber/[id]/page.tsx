...
export default async function GuidePreview({ params }: { params: Promise<{ id: string }> }) {
  ...
  const staged = guide.status === "published"
    ? await getCmsDraft<Draft>(pageDraftKey(guide.id)).catch(() => null)
    : null;
  ...
  return (
    <section className="content-editor-page">
      <div className="content-page-heading">
        <div><span>Önizleme · {locale.toUpperCase()}</span><h1>{title}</h1><p>Rehber taslağı yalnız CMS içinde gösterilir. Public rehber son yayınlanmış sürümde kalır.</p></div>
        <div className="content-form-actions"><Link href={`/icerik/rehber/${guide.id}?dil=${locale}`}>← Düzenlemeye dön</Link></div>
      </div>
      <div className="cms-preview-shell">
        <div className="cms-preview-banner"><span>Taslak Önizleme · Public Değil</span><span>{staged ? "Bekleyen çalışma taslağı" : `Kayıt durumu: ${guide.status}`}</span></div>
        <article className="cms-preview-article">
          <header><span className="cms-preview-eyebrow">İlkOku Rehber</span><h1>{title}</h1>{summary ? <p>{summary}</p> : null}</header>
          <Body body={body} />
        </article>
      </div>
    </section>

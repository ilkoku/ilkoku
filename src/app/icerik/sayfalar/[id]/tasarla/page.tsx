...
import {
  CmsVisualPageBuilder,
  type CmsVisualBuilderMediaOption,
} from "@/components/content/CmsVisualPageBuilder";
...
function parseMedia(row: MediaRow): CmsVisualBuilderMediaOption | null {
  try {
    ...
    if (
      ...
      !value.url.startsWith("/api/media/")
    ) {
      return null;
    }
    return {
...
function draftBlocks(value: unknown, fallback: CmsPageBlock[]) {
  ...
  try {
    return normalizeCmsPageBlocks(value);
  } catch {
    return fallback;
  }
}

export default async function VisualCmsPageEditor({
  params,
  searchParams,
}: {
  params: Promise<{ id: string }>;
  searchParams: Promise<{ hata?: string; kayit?: string; taslak?: string }>;
}) {
    ...
    return (
      <section className="content-editor-page">
        <div className="content-panel cms-editor-notice is-danger" role="alert">
          <strong>Görsel taslak bütünlüğü bozuk.</strong>
          <p>Ham taslak korunuyor; görsel oluşturucu kayıt üzerine yazmıyor.</p>
  ...
  const media = mediaRows
    .map(parseMedia)
    .filter((item): item is CmsVisualBuilderMediaOption => Boolean(item));

  return (
    <section className="content-editor-page" style={{ maxWidth: "none" }}>
      <div className="content-page-heading">
        <div>
          <span>Kurumsal Sayfa · Görsel Oluşturucu</span>
          <h1>{draft?.title ?? page.title}</h1>
          <p>
            Blokları sürükleyin, içerikleri düzenleyin ve aynı ekranda canlı sonucu izleyin.
            Header/footer ve site kimliği ortak sistemden gelir.
          </p>
        </div>
        <aside
          className="cms-editor-status-card"
          data-tone={page.status === "published" ? "success" : "info"}
        >
          <span className="cms-editor-status-card__label">Durum</span>
          <strong>
            {page.status === "published"
              ? draft
                ? "Yayında · taslak düzenleniyor"
                : "Yayında"
          ...
          <div className="cms-editor-status-card__meta">
            <span className="cms-editor-chip is-positive">{blocks.length} blok</span>
            <span className={`cms-editor-chip ${access.canPublish ? "is-positive" : "is-warning"}`}>
              {access.canPublish ? "Yayın yetkisi" : "Taslak yetkisi"}
            </span>
          </div>
        </aside>
      </div>

      <nav className="cms-editor-toolbar" aria-label="Görsel sayfa hızlı işlemleri">
        <div className="cms-editor-toolbar__cluster">
          <Link href="/icerik/sayfalar">← Sayfa listesi</Link>
          <span>Canlı önizleme sağ panelde</span>
        </div>
        <div className="cms-editor-toolbar__cluster">
          {page.status === "published" ? (
          ...
          <Link href="/icerik/seo">SEO</Link>
        </div>
      </nav>

      {query.hata === "kalite" ? (
        <div className="content-panel cms-editor-notice is-danger">
          <strong>Yayın kalite kapısı geçilemedi.</strong>
          <p>Canlı sürüm değiştirilmedi. İçerik/özet/SEO eksiklerini tamamlayıp yeniden yayınlayın.</p>
        </div>
      ) : null}
      {query.hata === "blok" ? (
        <div className="content-panel cms-editor-notice is-danger">
          <strong>Blok verisi kaydedilemedi.</strong>
          <p>Geçersiz blok, bağlantı veya Medya kütüphanesinde yayınlanmamış bir görsel bulundu.</p>
        </div>
      ) : null}
      {query.kayit || query.taslak ? (
        <div className="content-panel cms-editor-notice is-info">
          <strong>{query.taslak ? "Çalışma taslağı kaydedildi." : "Sayfa kaydedildi."}</strong>
          <p>
            {page.status === "published" && query.taslak
              ? "Canlı sürüm korunuyor; bu çalışma açıkça yayınlanana kadar ziyaretçilere gösterilmez."
              : "Görsel sayfa verisi ve sürüm geçmişi güncellendi."}
          </p>
        </div>
      ) : null}
      ...
      <div style={{ marginTop: "1rem" }}>
        <CmsVisualPageBuilder
          ...
          noIndex={draft?.noIndex ?? page.noIndex}
          canPublish={access.canPublish}
          defaultEyebrow={identity.defaultEyebrow}
          media={media}
        />
      </div>
    </section>

...
export async function AdminDashboard() {
  const [
    ...
    prisma.roleRequest.count({
      where: { requestedRole: "editor", status: "pending" },
    }),
    prisma.roleRequest.count({
      where: { requestedRole: "publisher", status: "pending" },
    }),
  ]);

  const pendingApplications =
    pendingEditorRequests + pendingPublisherRequests;
  ...
  const metrics = [
    ...
    },
    {
      ...
      delta: totalUsers
        ...
        : "%0",
      detail: "Hesabı aktif üyeler",
      tone: "violet",
      icon: "spark",
    },
    {
      label: "Yazar",
      value: writers.toLocaleString("tr-TR"),
      delta: "Canlı veri",
      detail: "Yazar rolündeki üyeler",
      tone: "cyan",
      icon: "book",
    },
    {
      label: "Editör Başvurusu",
      value: pendingEditorRequests.toLocaleString("tr-TR"),
      delta: pendingEditorRequests ? "İncelenmeli" : "Temiz",
      detail: "Bekleyen başvurular",
      tone: "amber",
      icon: "editor",
    },
    {
      label: "Yayınevi Başvurusu",
      value: pendingPublisherRequests.toLocaleString("tr-TR"),
      delta: pendingPublisherRequests ? "İncelenmeli" : "Temiz",
      detail: "Bekleyen başvurular",
      tone: "rose",
      icon: "building",
    },
    ...
    <>
      <section className="admin-page-heading">
        <div>
          ...
          <h1>Günaydın, Yönetici.</h1>
          <p>
            İlkOku ekosisteminin performansı, büyümesi ve bekleyen yönetim
            aksiyonları.
          </p>
        </div>
      ...
      <section className="admin-dashboard-grid admin-dashboard-grid--analytics">
        <article className="admin-panel admin-panel--wide">
          <div className="admin-panel__heading">
            <div>
              <span>Platform hareketliliği</span>
              <h2>Haftalık büyüme</h2>
            </div>
          ...
          <div
            ...
            role="img"
            aria-label="Okuma ve yeni üye sayılarını gösteren haftalık örnek grafik"
          >
            <div className="admin-bar-chart__scale">
              <span>1.3K</span>
              <span>975</span>
              <span>650</span>
              <span>325</span>
            ...
            <div className="admin-bar-chart__plot">
              {weekly.map((item) => (
                <div className="admin-bar-chart__group" key={item.day}>
                  <div className="admin-bar-chart__bars">
                    <i style={{ height: `${(item.reads / max) * 100}%` }} />
                    <i style={{ height: `${(item.members / max) * 100}%` }} />
                  </div>
                  <span>{item.day}</span>
                </div>
              ))}
        ...
        <article className="admin-panel admin-panel--tasks">
          ...
          <div className="admin-task-list">
            <button>
              <span className="admin-task-icon">E</span>
              <div>
                <strong>Editör başvuruları</strong>
                <small>
                  {pendingEditorRequests} başvuru incelenmeyi bekliyor
                </small>
              </div>
              <em>{pendingEditorRequests}</em>
              <i>→</i>
            </button>

            <button>
              <span className="admin-task-icon">Y</span>
              <div>
                <strong>Yayınevi başvuruları</strong>
                <small>
                  {pendingPublisherRequests} başvuru incelenmeyi bekliyor
                </small>
              </div>
              <em>{pendingPublisherRequests}</em>
              <i>→</i>
            </button>

            <button>
              <span className="admin-task-icon">K</span>
              <div>
                <strong>Kullanıcı yönetimi</strong>
                <small>{totalUsers} kayıtlı kullanıcı bulunuyor</small>
              </div>
              <em>{totalUsers}</em>
              <i>→</i>
            </button>
      ...
      <section className="admin-insight-grid">
        <article className="admin-panel">
          <div className="admin-panel__heading">
            <div>
              <span>İçerik performansı · örnek görünüm</span>
              <h2>En çok okunan eserler</h2>
            </div>
            <button>Tüm eserler</button>
          </div>

          <div className="admin-ranking-list">
            {popularWorks.map((work, index) => (
              <div className="admin-ranking" key={work.title}>
        ...
        <article className="admin-panel">
          <div className="admin-panel__heading">
            <div>
              <span>Topluluk · örnek görünüm</span>
              <h2>Yükselen yazarlar</h2>
            </div>
            <button>Tüm yazarlar</button>
          </div>

          <div className="admin-author-list">
            {authors.map((author) => (
              <div className="admin-author" key={author.name}>
                <span>{author.initials}</span>
                <div>
                  <strong>{author.name}</strong>
                  <small>
                    {author.genre} · {author.followers} takipçi
                  </small>
                </div>
                <b>{author.change}</b>
              </div>
          ...
          <div className="admin-mini-callout">
            ...
            <small>Canlı kullanıcı verisinden hesaplandı</small>
          </div>
        </article>
        ...
        <article className="admin-panel admin-panel--activity">
          <div className="admin-panel__heading">
            <div>
              <span>Canlı akış · örnek görünüm</span>
          ...
          <div className="admin-activity-list">
            {activities.map((activity) => (
              <div className="admin-activity" key={activity.title}>
                <span
                  className={`admin-activity__dot admin-activity__dot--${activity.tone}`}
                />
                <div>
      ...
      <section className="admin-quick-grid">
        <article className="admin-panel admin-panel--spotlight">
          <div>
            <span className="admin-eyebrow">Yönetim ilkesi</span>
            <h2>Yazarların hakları, platformun temelidir.</h2>
            <p>
              Telif hakkı daima yazarda kalır. Eser işlemleri şeffaflık ve
              denetlenebilirlik ilkesiyle kaydedilir.
            </p>
          </div>

          <button className="admin-button admin-button--light">
        ...
        <article className="admin-panel admin-panel--quick">
          ...
          <div>
            <button>
              Eser incele <span>→</span>
            </button>
            <button>
              Başvuru değerlendir <span>→</span>
            </button>
            <button>
              Duyuru yayımla <span>→</span>
            </button>
          </div>
        </article>
      </section>

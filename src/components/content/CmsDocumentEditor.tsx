  ...
  return (
    <section className="content-editor-page">
      <div className="content-page-heading">
        ...
        <div className="content-profile"><strong>{statusLabel}</strong><small>{revisionCount} CMS sürümü</small></div>
      </div>

      <div className="content-form-actions" style={{ marginBottom: "1rem", flexWrap: "wrap" }}>
        <Link href={`/icerik/yasal/${slug}?dil=tr`}>Türkçe</Link><Link href={`/icerik/yasal/${slug}?dil=en`}>English</Link>
      ...
      {corruptDraft ? <div className="content-panel" role="alert" style={{ marginBottom: "1rem" }}><strong>Çalışma taslağının bütünlüğü bozuk.</strong><p>Ham yasal belge taslağı korunuyor. Veri doğrulanmadan düzenleme, önizleme ve yayınlama işlemleri durduruldu; mevcut canlı metin değişmedi.</p><div className="content-form-actions"><Link href="/icerik/saglik">Sistem Sağlığı →</Link><Link href="/icerik/gecmis">Revision Center →</Link></div></div> : null}
      {hasPendingDraft ? <div className="content-panel" style={{ marginBottom: "1rem" }}><strong>Bekleyen çalışma taslağı var.</strong><p>Public sayfa hâlâ son yayınlanmış sürümü gösteriyor. Taslağı önizleyebilir veya yayın yetkisiyle canlıya aktarabilirsiniz.</p></div> : null}
      {isEn && !localeEnabled && !corruptDraft ? <div className="content-panel" style={{ marginBottom: "1rem" }}><strong>EN yayın kilitli.</strong><p>Bu belge taslak olarak kaydedilebilir ve önizlenebilir. İngilizce public dili açılmadan yayınlanamaz.</p></div> : null}
      ...
      {!corruptDraft ? <div className="content-panel">
        <form action={saveCmsDocumentAction} className="content-form">
          ...
          <div className="content-form-actions"><button type="submit" name="mode" value="draft">Taslak Kaydet</button>{canPublish ? <button type="submit" name="mode" value="publish">Kaydet ve Yayınla</button> : null}</div>
        </form>
      </div> : null}
    </section>
  );

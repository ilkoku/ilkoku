  ...
  return (
    <section className="content-editor-page">
      <div className="content-page-heading">
        ...
        <div className="content-profile"><strong>{statusLabel}</strong><small>{guide ? `${revisionCount} sürüm` : "Yeni içerik"}</small></div>
      </div>

      <div className="content-form-actions" style={{ marginBottom: "1rem", flexWrap: "wrap" }}>
        <Link href="/icerik/rehber?dil=tr">Türkçe</Link><Link href="/icerik/rehber?dil=en">English</Link>
      ...
      {corruptDraft ? <div className="content-panel" role="alert" style={{ marginBottom: "1rem" }}><strong>Çalışma taslağının bütünlüğü bozuk.</strong><p>Ham taslak korunuyor. Veri doğrulanmadan düzenleme, önizleme, yayınlama ve arşivleme durduruldu; canlı rehber değişmedi.</p><div className="content-form-actions"><Link href="/icerik/saglik">Sistem Sağlığı →</Link><Link href="/icerik/gecmis">Revision Center →</Link></div></div> : null}

      {hasPendingDraft ? <div className="content-panel" style={{ marginBottom: "1rem" }}><strong>Bekleyen çalışma taslağı var.</strong><p>Public rehber hâlâ son yayınlanmış sürümü gösteriyor. Taslağı önizleyebilir veya yayın yetkisiyle canlıya aktarabilirsiniz.</p></div> : null}
      {isEn && !localeEnabled && !corruptDraft ? <div className="content-panel" style={{ marginBottom: "1rem" }}><strong>EN yayın kilitli.</strong><p>Rehber taslak olarak kaydedilebilir, önizlenebilir ve sürümlenebilir. İngilizce public dili açılmadan yayınlanamaz.</p></div> : null}
      ...
      {!corruptDraft ? <div className="content-panel">
        <form action={saveCmsGuideAction} className="content-form">
          ...
          <div className="content-form-actions" style={{ justifyContent: "space-between", flexWrap: "wrap" }}><Link href={`/icerik/rehber?dil=${locale}`}>← Rehber listesi</Link><div style={{ display: "flex", gap: ".7rem", flexWrap: "wrap" }}><button type="submit" name="mode" value="draft">Taslak Kaydet</button>{canPublish ? <button type="submit" name="mode" value="publish">Kaydet ve Yayınla</button> : null}</div></div>
        </form>
        {guide ? <div className="content-publish-box"><div><strong>Arşivleme</strong><p>Arşivlenen rehber public siteden kaldırılır; sürüm geçmişi korunur.</p></div><form action={archiveCmsGuideAction}><input type="hidden" name="id" value={guide.id} /><input type="hidden" name="locale" value={locale} /><button type="submit">Arşivle</button></form></div> : null}
      </div> : null}
    </section>
  );
}

...
export function HowItWorksExperience({ body, summary, title, updatedAt }: HowItWorksExperienceProps) {
  ...
  return (
    <main className="how-page">
      ...
      <section className="how-hero">
        <div className="how-container how-hero__grid">
          ...
          <figure className="how-hero__visual">
            ...
            <figcaption><span><JourneyIcon name="draft" /> Yazar oluşturur</span><span><JourneyIcon name="review" /> Editörler değerlendirir</span><span><JourneyIcon name="discover" /> Okur ve yayınevi keşfeder</span></figcaption>
          </figure>
        </div>
      </section>
      ...
      {parsed.intro ? <section className="how-truth how-container" aria-label="Platform sınırları"><span><JourneyIcon name="passport" /></span><div><strong>Tanıtım vaadi değil, gerçek çalışma sınırları.</strong><EditorialBody body={parsed.intro} /></div></section> : null}

      {sectionMap.get("İlkOku nedir?") ? <section className="how-definition how-container"><div><SectionHeading eyebrow="Platformun temeli" title="Eser yalnız son hâliyle değil, yolculuğuyla değer kazanır." /><EditorialBody body={sectionMap.get("İlkOku nedir?")!.body} /></div><aside><JourneyIcon name="publish" /><strong>Aynı eser, ayrı yetkiler</strong><p>Taslak, yayın, editör incelemesi ve yayınevi keşfi birbirinden ayrı kararlardır.</p></aside></section> : null}

      {journey ? <section className="how-journey" id={headingId(journey.title)}><div className="how-container"><SectionHeading eyebrow="Baştan sona gerçek akış" title={journey.title} description="Her adım bir öncekinin devamıdır; görünürlük ve yetki sınırları süreç boyunca korunur." />{journeyContent.intro ? <div className="how-section-intro"><EditorialBody body={journeyContent.intro} /></div> : null}<div className="how-journey__grid">{journeyParts.map((part, index) => <article className={index === 7 ? "how-step how-step--passport" : "how-step"} key={part.title} id={headingId(part.title)}><div className="how-step__top"><span className="how-step__number">{String(index + 1).padStart(2, "0")}</span><span className="how-step__icon"><JourneyIcon name={iconForStep(index)} /></span></div><h3>{part.title.replace(/^\d+\.\s*/, "")}</h3><EditorialBody body={part.body} />{index === 7 ? <div className="how-step__passport-line"><i /><span>Oluşturma</span><i /><span>Sürümler</span><i /><span>İnceleme</span></div> : null}</article>)}</div></div></section> : null}

      {roleSection ? <section className="how-roles how-container" id={headingId(roleSection.title)}><SectionHeading eyebrow="Dört rol, açık sorumluluklar" title={roleSection.title} description="Herkes aynı platformdadır; ancak aynı bilgiye ve aynı işleme erişmez." />{roleContent.intro ? <div className="how-section-intro"><EditorialBody body={roleContent.intro} /></div> : null}<div className="how-roles__grid">{roles.map((role) => { const content = roleParts.find((part) => part.title === role.title); return <article className="how-role" key={role.title} id={headingId(role.title)}><div className="how-role__header"><Image src={role.asset} alt="" aria-hidden="true" width={74} height={74} /><div><span>{role.label} rolü</span><h3>{role.label}</h3></div></div>{content ? <EditorialBody body={content.body} /> : null}<Link href={role.href}>{role.label} alanına git <span aria-hidden="true">→</span></Link></article>; })}</div>{unknownRoleParts.length > 0 ? <div className="how-extras how-role-extras">{unknownRoleParts.map((part) => <GenericSection section={part} key={part.title} />)}</div> : null}</section> : null}

      {sectionMap.get("Kim neyi görebilir?") ? <section className="how-visibility" id="kim-neyi-gorebilir"><div className="how-container"><SectionHeading eyebrow="Sunucu tarafında korunan sınırlar" title="Kim neyi görebilir?" description="Public bilgi ile yetki gerektiren çalışma kaydı aynı görünürlük düzeyinde değildir." /><div className="how-visibility__panel"><EditorialBody body={sectionMap.get("Kim neyi görebilir?")!.body} /></div></div></section> : null}
      ...
      <section className="how-guardrails how-container">
        ...
        {sectionMap.get("İlkOku ne yapmaz?") ? <GenericSection section={sectionMap.get("İlkOku ne yapmaz?")!} tone="night" /> : null}
      </section>

      {sectionMap.get("Özelliklerin güncel durumu") ? <section className="how-status how-container"><SectionHeading eyebrow="Şeffaf ürün durumu" title="Bugün hangi özellik hangi aşamada?" /><GenericSection section={sectionMap.get("Özelliklerin güncel durumu")!} /></section> : null}
      ...
      {sectionMap.get("Nereden başlamalısınız?") ? <section className="how-start" id="nereden-baslamalisiniz"><div className="how-container how-start__grid"><div><span className="how-eyebrow">Sıra sende</span><h2>Nereden başlamalısınız?</h2><EditorialBody body={sectionMap.get("Nereden başlamalısınız?")!.body} /></div><aside><strong>Eserinin ilk kaydını oluştur.</strong><p>Taslak olarak başla; ne zaman yayımlanacağına sen karar ver.</p><Link className="how-button how-button--primary" href="/kayit?rol=writer">Yazar olarak katıl <span aria-hidden="true">→</span></Link><Link href="/eserler">Önce eserleri keşfet</Link></aside></div></section> : null}

      <aside className="how-related how-container" aria-label="İlkOku içinde devam et"><SectionHeading eyebrow="İlkOku içinde devam et" title="Platformu kendi yolundan keşfet." /><div className="how-related__grid"><Link href="/eserler"><strong>Eserleri keşfet</strong><span>Herkese açık yayımlanmış eserleri incele.</span></Link><Link href="/yazarlar"><strong>Yazarları keşfet</strong><span>Yazarların açık profillerinden yayımlanmış eserlerine ulaş.</span></Link><Link href="/editorler"><strong>Editörleri incele</strong><span>Herkese açık editör profillerini ve çalışma alanlarını gör.</span></Link><Link href="/yardim"><strong>Yardım Merkezi</strong><span>Hesap, roller ve platform kullanımı hakkında yanıt bul.</span></Link></div></aside>

      <footer className="how-footer"><div className="how-container"><Link className="how-logo" href="/"><Image src={logo} alt="İlkOku" sizes="150px" /></Link><nav><Link href="/nasil-calisir">Nasıl Çalışır?</Link><Link href="/editoryal-standartlar">Editoryal Standartlar</Link><Link href="/eserler">Eserler</Link><Link href="/yasal/gizlilik-politikasi">Gizlilik</Link><Link href="/yasal/kullanim-sartlari">Kullanım Şartları</Link></nav></div></footer>
    </main>
  );
}

...
export function MediaLibraryWorkbench({
  ...
  const selectedDimensions = selected ? dimensions[selected.contentKey] : undefined;
  ...
  const canDeleteSelected = Boolean(selected && canPublish && referencesAvailable && selected.references.length === 0);
  const deleteLockReason = !canPublish
    ? "Silme için yayın yetkisi gerekiyor."
    : !referencesAvailable
      ? "Kullanım haritası doğrulanamadı."
      : selected && selected.references.length > 0
        ? `Bu medya ${selected.references.length} canlı yerde kullanılıyor.`
        : "";
  ...
  function registerDimensions(contentKey: string, width: number, height: number) {
    if (!width || !height) return;
    setDimensions((current) => {
      const existing = current[contentKey];
      if (existing?.width === width && existing?.height === height) return current;
      return { ...current, [contentKey]: { width, height } };
    });
  }
  ...
  return (
    <div className={styles.workbench}>
      <div className={styles.summaryBar}>
        ...
        <article><span>Silinebilir</span><strong>{referencesAvailable ? unusedCount : "—"}</strong><small>canlı referansı yok</small></article>
      ...
      <div className={styles.layout}>
        <aside className={styles.rail}>
          ...
          <div className={styles.filters}>
            <label><span>Arama</span><input name="q" value={search} onChange={(event) => setSearch(event.target.value.slice(0, 120))} placeholder="Dosya, başlık, kullanım..." /></label>
            <div className={styles.filterGrid}>
              ...
              <label><span>Kullanım</span><select name="kullanim" value={usage} disabled={!referencesAvailable} onChange={(event) => setUsage(normalizedUsage(event.target.value))}><option value="all">Tümü</option><option value="used">Kullanımda</option><option value="unused">Silinebilir / boşta</option></select></label>
            </div>
            ...
            {filtered.length === 0 ? <div className={styles.railEmpty}><strong>Sonuç yok</strong><small>Aramayı veya filtreleri değiştir.</small></div> : filtered.map((asset) => {
              ...
              return (
                <button key={asset.contentKey} type="button" className={`${styles.assetItem} ${active ? styles.assetItemActive : ""}`} onClick={() => selectAsset(asset.contentKey)}>
                  ...
                  <span className={`${styles.usageBadge} ${asset.references.length > 0 ? styles.used : styles.unused}`}>{referencesAvailable ? asset.references.length > 0 ? `${asset.references.length} kullanım` : "Silinebilir" : "?"}</span>
                </button>
              );
            })}
          </div>
          ...
          {mode === "new" ? (
            ...
            <>
              <div className={styles.detailHeading}>
                ...
                <div className={`${styles.detailActions} cms-media-detail-actions`}>
                  <CopyMediaUrlButton url={selected.url} />
                  <Link href={selected.url} target="_blank">Dosyayı aç ↗</Link>
                  {canDeleteSelected ? (
                    <form action={archiveMediaAssetAction} onSubmit={(event) => { if (!window.confirm("Bu medya çöp kutusuna taşınacak ve public blob erişimi kapatılacak. Devam edilsin mi?")) event.preventDefault(); }}>
                      <input type="hidden" name="contentKey" value={selected.contentKey} />
                      <button type="submit" className="cms-media-delete-button">Sil</button>
                    </form>
                  ) : <button type="button" className="cms-media-delete-button is-disabled" disabled title={deleteLockReason}>Sil</button>}
                </div>
              </div>

              <div className={styles.previewStage}>
                {selected.kind === "image" || selected.kind === "icon" ? <Image src={selected.url} alt={selected.altText || selected.title || "İlkOku medya"} width={1200} height={760} unoptimized onLoad={(event) => registerDimensions(selected.contentKey, event.currentTarget.naturalWidth, event.currentTarget.naturalHeight)} /> : <div className={styles.documentPreview}><span>{selected.kind === "document" ? "PDF" : "DOSYA"}</span><strong>{selected.filename || selected.title}</strong><small>{selected.mimeType || kindLabel(selected.kind)}</small></div>}
              </div>

              <div className={`${styles.metadataGrid} cms-media-metadata-grid`}>
                <article><span>Tür</span><strong>{selected.mimeType || kindLabel(selected.kind)}</strong></article>
                <article><span>Kaynak ölçü</span><strong>{selectedDimensions ? `${selectedDimensions.width}×${selectedDimensions.height} px` : selected.kind === "document" ? "—" : "Yükleniyor…"}</strong></article>
                <article><span>Kaynak oran</span><strong>{ratioLabel(selectedDimensions)}</strong></article>
                <article><span>Dosya boyutu</span><strong>{formatBytes(selected.sizeBytes)}</strong></article>
                <article><span>Kullanıldığı alan</span><strong>{selected.targetLabel || selected.usage || (selected.references.length > 0 ? "Canlı içerik alanı" : "Tanımlı değil")}</strong></article>
                <article><span>Hedef ölçü</span><strong>{selected.targetWidth && selected.targetHeight ? `${selected.targetWidth}×${selected.targetHeight} px` : selected.references.length > 0 ? "Responsive / bileşene bağlı" : "Tanımlı değil"}</strong></article>
                <article><span>Hedef oran / fit</span><strong>{selected.targetAspectRatio ? `${selected.targetAspectRatio}${selected.fit ? ` · ${selected.fit}` : ""}` : selected.fit || "—"}</strong></article>
              ...
              <div className={styles.contentInfo}>
                ...
                <div><span>Güncelleme</span><strong>{formatDate(selected.updatedAt)}</strong></div>
        ...
        <aside className={styles.sidePane}>
          {mode === "new" ? (
            <div className={styles.sideStack}>
              ...
              <div className={styles.sideSection}><span>Sonraki adım</span><strong>Yüklenen medya hemen envantere eklenir</strong><p>Sonrasında gerçek kullanım yerlerini, ölçüleri ve silme uygunluğunu aynı ekrandan görebilirsin.</p></div>
            </div>
          ) : selected ? (
            <div className={styles.sideStack}>
              <div className={styles.sideSection}>
                ...
                {!referencesAvailable ? <p className={styles.warningText}>Kullanım haritası doğrulanamadığı için silme fail-closed olarak kilitli.</p> : null}
              </div>
              ...
              <div className={`${styles.archivePanel} cms-media-delete-panel`}>
                <span>Silme güvenliği</span>
                {canDeleteSelected ? <><strong>Silinebilir</strong><p>Sil düğmesi kaydı ve blobu arşive taşır. İşlem sırasında server canlı referansları tekrar kontrol eder.</p><form action={archiveMediaAssetAction} onSubmit={(event) => { if (!window.confirm("Bu medya çöp kutusuna taşınacak. Devam edilsin mi?")) event.preventDefault(); }}><input type="hidden" name="contentKey" value={selected.contentKey} /><button type="submit">Sil / Çöp Kutusuna Taşı</button></form></> : <><strong>Silme kilitli</strong><p>{deleteLockReason}</p></>}
              </div>
            </div>
          ) : <div className={styles.sideSection}><span>Operasyon</span><strong>Bir medya seç</strong><p>Kullanım, ölçü ve güvenli silme kararları seçili kayıt üzerinden gösterilir.</p></div>}
        </aside>
      </div>
    </div>
  );

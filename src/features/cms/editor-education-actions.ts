...
export async function saveEditorEducationTextAction(formData: FormData) {
  ...
  let access = await requireCmsManager(`/icerik/editor-egitim/${category.slug}`);
  if (mode === "publish") access = await requireCmsPublisher(`/icerik/editor-egitim/${category.slug}`);

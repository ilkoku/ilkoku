type HistoryArt = "writer" | "editor" | "publisher" | "film" | "desk";
...
function HistoricalArt({ kind, className = "" }: { kind: HistoryArt; className?: string }) {
  const common = {
    className: `landing-history-art landing-history-art--${kind} ${className}`.trim(),
    "aria-hidden": true,
  ...
  if (kind === "writer") {
    return (
      <svg {...common} viewBox="100 250 240 250">
        <defs>
          <clipPath id="history-writer-art-clip">
            <ellipse cx="216" cy="378" rx="104" ry="114" />
          </clipPath>
        </defs>
        <image href={sourceImage} x="0" y="0" width="1492" height="1054" clipPath="url(#history-writer-art-clip)" />
      </svg>
    );
  }
  ...
  if (kind === "editor") {
    return (
      <svg {...common} viewBox="350 335 265 310">
        <defs>
          <clipPath id="history-editor-art-clip">
        ...
        <image href={sourceImage} x="0" y="0" width="1492" height="1054" clipPath="url(#history-editor-art-clip)" />
      </svg>
    );
  }
  ...
  if (kind === "publisher") {
    return (
      <svg {...common} viewBox="590 220 410 400">
        <defs>
          <clipPath id="history-publisher-art-clip">
            <polygon points="598,236 986,226 991,574 970,597 611,603 596,578" />
          </clipPath>
        </defs>
        <image href={sourceImage} x="0" y="0" width="1492" height="1054" clipPath="url(#history-publisher-art-clip)" />
      </svg>
    );
  }
  ...
  if (kind === "film") {
    return (
      <svg {...common} viewBox="970 175 490 320">
        <defs>
          <clipPath id="history-film-art-clip">
            <polygon points="987,214 1431,184 1449,449 1000,488" />
          </clipPath>
        </defs>
        <image href={sourceImage} x="0" y="0" width="1492" height="1054" clipPath="url(#history-film-art-clip)" />
      </svg>
    );
  }
  ...
  return (
    <svg {...common} viewBox="0 718 725 336">
      <defs>
        <clipPath id="history-desk-art-clip">
          <polygon points="0,724 151,757 222,806 337,834 470,894 611,955 724,1015 724,1054 0,1054" />
        </clipPath>
      </defs>
      <image href={sourceImage} x="0" y="0" width="1492" height="1054" clipPath="url(#history-desk-art-clip)" />
    </svg>
  );
}
...
function JourneyIcon({ kind }: { kind: BenefitIcon }) {
  ...
  if (kind === "reader") {
    return <svg {...common}><path d="M3.5 5.5A3.5 3.5 0 0 1 7 2h4v17H7a3.5 3.5 0 0 0-3.5 3V5.5Z" /><path d="M20.5 5.5A3.5 3.5 0 0 0 17 2h-4v17h4a3.5 3.5 0 0 1 3.5 3V5.5Z" /></svg>;
  }

  if (kind === "editor") {
    return <svg {...common}><path d="M4 20h4l10.2-10.2a2.1 2.1 0 1 0-3-3L5 17l-1 3Z" /><path d="m13.8 8.2 3 3" /></svg>;
  }
...
const historyCards = [
  {
    key: "writer",
    art: "writer" as const,
    date: "MÖ 23. YÜZYIL · YAZ",
    title: "Enheduanna",
    strong: "Bir yazar, adını eserinin yanında bıraktı.",
    body: <>Binlerce yıl geçti.<br />Adı hâlâ okunuyor.</>,
  },
  {
    key: "editor",
    art: "editor" as const,
    date: "MÖ 3. YÜZYIL · GELİŞTİR",
    title: "Zenodotos",
    strong: "Birisi yazılmış bir metne yeniden baktı.",
    body: <>Çünkü bazen bir eser,<br />ikinci bir bakışla daha da güçlenir.</>,
  },
  {
    key: "publisher",
    art: "publisher" as const,
    date: "1534 · İNAN",
    title: <>Cambridge<br />University Press</>,
    strong: "Bir eserin dünyaya ulaşması için birilerinin ona inanması gerekiyordu.",
    body: <>Yazarın yolculuğuna yayıncı da katıldı.</>,
  },
  {
    key: "film",
    art: "film" as const,
    date: "1895 · HAYATA GEÇİR",
    title: "Hikâye perdeye çıktı.",
    strong: "Hikâyeler artık yalnızca okunmuyordu, izlenmeye de başlandı.",
    ...
    <section className="landing-history" id="hikayenin-yolculugu" aria-labelledby="history-heading">
      <div className="landing-container landing-history__container">
        ...
        <div className="landing-history__timeline">
          {historyCards.map((card) => (
            <article key={card.key} className={`landing-history-era landing-history-era--${card.key}`}>
              <div className="landing-history-era__art">
                <HistoricalArt kind={card.art} />
              </div>
        ...
        <div className="landing-history__closing-stage">
          <div className="landing-history__desk" aria-hidden="true">
            <HistoricalArt kind="desk" />
          </div>

          <article className="landing-history-now">
            <div className="landing-history-now__seal" aria-hidden="true">
              <span>▤</span>
            </div>

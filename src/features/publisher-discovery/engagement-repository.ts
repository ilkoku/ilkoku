...
import {
  requirePublisherMembershipPermission,
...
export async function setPublisherWorkLikeV2(input: {
  ...
  const membership =
    await requirePublisherMembershipPermission(
      input.userId,
      "like_work",
    );
  ...
  try {
    return await prisma.$transaction(
      async (transaction) => {
        const existing =
          await transaction.publisherWorkLike.findUnique({
            where: {
              publisherId_workId: {
                publisherId:
...
export async function setPublisherAuthorFollowV2(input: {
  ...
  try {
    return await prisma.$transaction(
      async (transaction) => {
        const existing =
          await transaction.publisherAuthorFollow.findUnique({
            where: {

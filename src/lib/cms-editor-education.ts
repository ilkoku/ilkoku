...
async function getEditorEducationTextRow(namespace: string, categorySlug: string) {
...
export async function getEditorEducationPublishedTextRecord(categorySlug: string) {
  const category = getEditorEducationCategory(categorySlug);
  if (!category) return null;
  const row = await getEditorEducationTextRow(EDITOR_EDUCATION_TEXT_NAMESPACE, category.slug);
...
export async function getEditorEducationDraftTextRecord(categorySlug: string) {
  const category = getEditorEducationCategory(categorySlug);
  if (!category) return null;
  const row = await getEditorEducationTextRow(EDITOR_EDUCATION_TEXT_DRAFT_NAMESPACE, category.slug);

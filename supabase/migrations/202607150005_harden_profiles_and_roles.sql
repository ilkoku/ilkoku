...
alter table public.profiles
  ...
  add column if not exists role_approved_by uuid;

alter table public.profiles alter column role set default 'reader'::public.user_role;
alter table public.profiles alter column updated_at set default timezone('utc'::text, now());

update public.profiles
set requested_role = role,
    role = 'reader'::public.user_role,
    role_approved_at = null,
    role_approved_by = null,
    updated_at = timezone('utc'::text, now())
where role in ('editor'::public.user_role, 'publisher'::public.user_role)
  and role_approved_at is null;

update public.profiles
set requested_role = null
where requested_role is not null
  and requested_role not in ('editor'::public.user_role, 'publisher'::public.user_role);

update public.profiles
set role_approved_at = null,
    role_approved_by = null
where role in ('reader'::public.user_role, 'writer'::public.user_role)
  and (role_approved_at is not null or role_approved_by is not null);

update public.profiles
set role_approved_by = null
where role_approved_by is not null
  and not exists (
    select 1 from auth.users where auth.users.id = profiles.role_approved_by
  );

alter table public.profiles drop constraint if exists profiles_role_approved_by_fkey;
alter table public.profiles
  add constraint profiles_role_approved_by_fkey
  foreign key (role_approved_by) references auth.users(id) on delete set null;
...
drop policy if exists "Kullanıcı profil bilgilerini güncelleyebilir" on public.profiles;
